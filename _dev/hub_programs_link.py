...
Add the programmes directory to the hub's question index."""
import os, re, sys

HERE = os.path.dirname(os.path.abspath(__file__))
SITE = os.path.dirname(HERE)
HUB, PAGE = "resources.html", "mft-programs-california.html"

ROW = ('<a class="qread" href="' + PAGE + '"><div>'
       '<h3>Which graduate programme should I go to?</h3>'
       '<div class="m"><span class="tag">Licensure</span>'
       '<span class="g">Read &middot; 65 CA programmes</span></div></div>'
...
def main():
    import sys as _s
    print("hub_programs_link.py: retired - the hub is generated from registry.json")
    _s.exit(0)

    path = os.path.join(SITE, HUB)
    if not os.path.exists(os.path.join(SITE, PAGE)):
        sys.exit("hub_programs_link: %s missing" % PAGE)
    s = open(path, encoding="utf-8").read()
    if ('href="%s"' % PAGE) in s:
        print("%-44s already linked" % HUB)
    else:
        if s.count(ANCHOR) != 1:
            sys.exit("hub_programs_link: anchor matched %d times" % s.count(ANCHOR))
        s = s.replace(ANCHOR, ROW + ANCHOR, 1)
        open(path, "w", encoding="utf-8").write(s)
        print("%-44s programmes added to the question index" % HUB)
    s = open(path, encoding="utf-8").read()
    bad = 0
    ...
    if bad:
        sys.exit("hub_programs_link: %d guard failure(s)" % bad)
